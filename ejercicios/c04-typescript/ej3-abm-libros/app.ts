// 1. LA INTERFAZ (Nuestro molde) [cite: 960]
interface Libro {
    isbn: string;
    titulo: string;
    autor: string;
...
const checkDisponible = document.querySelector('#checkDisponible') as HTMLInputElement;
...
function renderizar(): void {
    ...
    for (const libro of catalogo) {
        ...
        if (!libro.disponible) li.style.color = "red";
...
function validarFormulario(): Libro | null {
    ...
    return {
        isbn: "AUTO-" + Date.now(), // Genera un ID único con la fecha y hora
        titulo: titulo,
        autor: autor,
        precio: precio,
